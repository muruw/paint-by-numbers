...
import { pointToPolygonDist, polylabel } from "./lib/polylabel.ts";
    ...
    for (const f of facetResult.facets) {
      if (f != null) {
        ...
        for (const neighbourIdx of f.neighbourFacets!) {
          const neighbourPath = facetResult.facets[neighbourIdx]!
            .getFullPathFromBorderSegments(true);
          const fallsInside: boolean = FacetLabelPlacer
            .doesNeighbourFallInsideInCurrentFacet(
              neighbourPath,
              f,
              onlyOuterRing,
            );
