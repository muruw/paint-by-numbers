...
import { BooleanArray2D } from "./structs/typedarrays.ts";
import {
  Facet,
  FacetResult,
  OrientationEnum,
  PathPoint,
} from "./facetmanagement.ts";
...
export class FacetBorderTracer {
  ...
  public static async buildFacetBorderPaths(
    ...
    const facetProcessingOrder = facetResult.facets.filter((f) => f != null)
      .slice(0).sort((a, b) =>
        b!.pointCount > a!.pointCount
          ? 1
          : (b!.pointCount < a!.pointCount ? -1 : 0)
      ).map((f) => f!.id);
  ...
  private static getPath(
    ...
    while (!finished) {
      ...
      if (pt.orientation === OrientationEnum.Left) {
        ...
        //   +---x---+
        if (
          pt.y - 1 >= 0 && pt.x - 1 >= 0 && // there is a left upwards
        ...
        //   +---+---+
        if (
          pt.y + 1 < facetResult.height && pt.x - 1 >= 0 && // there is a left downwards
        ...
        // check right upwards
        if (
          pt.y - 1 >= 0 && pt.x + 1 < facetResult.width && // there is a right upwards
          facetResult.facetMap.get(pt.x + 1, pt.y - 1) === f.id && // and it belongs to the same facet
        ...
        if (
          pt.y + 1 < facetResult.height && pt.x + 1 < facetResult.width && // there is a right downwards
          facetResult.facetMap.get(pt.x + 1, pt.y + 1) === f.id && // and belongs to the same facet
          borderMask.get(pt.x + 1, pt.y + 1) && // and is on the border
