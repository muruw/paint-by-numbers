...
import { Uint32Array2D } from "./structs/typedarrays.ts";
import { Point } from "./structs/point.ts";
import { FacetBoundarySegment } from "./facetBorderSegmenter.ts";
    ...
    for (const seg of this.borderSegments) {
      ...
      if (lastSegment != null) {
        if (lastSegment.reverseOrder) {
          ...
          addPoint(
            lastSegment.originalSegment
              .points[lastSegment.originalSegment.points.length - 1],
          );
        }
      }
