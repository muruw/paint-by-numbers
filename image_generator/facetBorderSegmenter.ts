import { FacetResult, OrientationEnum, PathPoint } from "./facetmanagement.ts";
...
export class FacetBorderSegmenter {
    ...
    for (const f of facetResult.facets) {
      if (f != null) {
        ...
        if (f.borderPath.length > 1) {
          ...
          // new segment
          if (currentPoints.length > 1) {
            const oldNeighbour = f.borderPath[f.borderPath.length - 1]
  ...
  private static isOutsideBorderPoint(
    ...
    return point.x === 0 || point.y === 0 || point.x === width - 1 ||
      point.y === height - 1;
  }
      ...
      if (f != null) {
        ...
        for (let s: number = 0; s < segmentsPerFacet[f.id].length; s++) {
          ...
          if (segment != null && f.borderSegments[s] == null) {
            ...
            if (debug) {
              console.log(
                "Setting facet " + f.id + " segment " + s + " to " +
                  f.borderSegments[s],
              );
            }
            if (segment.neighbour !== -1) {
              ...
              if (neighbourFacet != null) {
                ...
                for (let ns: number = 0; ns < neighbourSegments.length; ns++) {
                  ...
                  if (
                    ...
                    const nSegEndPoint = neighbourSegment
                      .points[neighbourSegment.points.length - 1];
                    let matchesStraight =
                    ...
                    if (matchesStraight) {
                      ...
                      if (debug) {
                        console.log(
                          "Match found for facet " + f.id + " to neighbour " +
                            neighbourFacet.id,
                        );
                      }
                      ...
                      if (debug) {
                        console.log(
                          "Setting facet " + neighbourFacet!.id + " segment " +
                            ns + " to " + neighbourFacet!.borderSegments[ns],
                        );
                      }
                      ...
                      if (debug) {
                        console.log(
                          "Reverse match found for facet " + f.id +
                            " to neighbour " + neighbourFacet.id,
                        );
                      }
                      neighbourFacet!.borderSegments[ns] =
              ...
              if (!matchFound && debug) {
                ...
                // cover all cases
                console.error(
                  "No match found for segment of " + f.id + ": " +
                    ("siding " + segment.neighbour + " " + segment.points[0] +
                      " -> " + segment.points[segment.points.length - 1]),
                );
              }
            }
