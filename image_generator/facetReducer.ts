...
import { BooleanArray2D, Uint8Array2D } from "./structs/typedarrays.ts";
    ...
    for (let y: number = facet.bbox.minY; y <= facet.bbox.maxY; y++) {
      for (let x: number = facet.bbox.minX; x <= facet.bbox.maxX; x++) {
        if (facetResult.facetMap.get(x, y) === facet.id) {
          ...
          needsToRebuild = true;
          if (
            x - 1 >= 0 && facetResult.facetMap.get(x - 1, y) !== facet.id &&
            ...
            y - 1 >= 0 && facetResult.facetMap.get(x, y - 1) !== facet.id &&
