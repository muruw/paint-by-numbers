 ...
 */
import { delay, IMap, RGB } from "./common.ts";
import { KMeans, Vector } from "./lib/clustering.ts";
import { hslToRgb, lab2rgb, rgb2lab, rgbToHsl } from "./lib/colorconversion.ts";
import { Uint8Array2D } from "./structs/typedarrays.ts";
import { Random } from "./random.ts";
import { ClusteringColorSpace, Settings } from "./const/Settings.ts";
...
export class ColorReducer {
  ...
  public static async applyKMeansClustering(
    ...
    for (const color of Object.keys(pointsByColor)) {
      ...
      const weight = pointsByColor[color].length /
  ...
  public static updateKmeansOutputImageData(
    ...
    for (let c: number = 0; c < kmeans.centroids.length; c++) {
      ...
      for (const v of kmeans.pointsPerCategory[c]) {
        ...
        const pointColor = `${Math.floor(pointRGB[0])},${
          Math.floor(pointRGB[1])
